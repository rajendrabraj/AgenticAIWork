...
os.environ["google_cloud_api_key"] = os.getenv("google_cloud_api_key")
...
def google_search(query: str, num_results: int = 2, max_chars: int = 500) -> list:  # type: ignore[type-arg]
    ...
    load_dotenv()
    print("Invoked the Google Search Function... \n")
    api_key = os.getenv("google_cloud_api_key")
    print("API Key : " + api_key)
    search_engine_id = os.getenv("MY_GOOGLE_ENGINE")
    print(search_engine_id)
    print("Search Engine : " + search_engine_id)
    ...
    print("COMPLETED the Google Search Function... \n")
...
def analyze_stock(ticker: str) -> dict:  # type: ignore[type-arg]
    ...
    print("Invoked the Analyze Stock Function... \n")
    print(ticker)
    
    # ticket_input = input("Enter STOCK TICKER NAME : ")
    # print(ticket_input)
    # stock = yf.Ticker(ticket_input)
   ...
   # Convert numpy types to Python native types for better JSON serialization
    ...
    print("Processed Results the Analyze Stock Function... \n")
    return result
...
stock_analysis_tool = FunctionTool(analyze_stock, description="Analyze stock data")
...
stock_analysis_agent = AssistantAgent(
    ...
    description="Provide information about TATAMOTORS and Analyze stock data and generate a plot",
    system_message="Perform data analysis and show historical data.",
)
...
##Set the Round Robin Team.

#my_increment_team = RoundRobinGroupChat(participants=[add_1_agent_first,add_1_agent_second,add_1_agent_third],max_turns=6)
my_team_one = RoundRobinGroupChat([stock_analysis_agent, search_agent, report_agent], max_turns=3)
# ##Resume a team
async def my_best_search():
    print("STARTED BEST SEARCH \n")
    #stream = my_team_one.run_stream(task="Write a financial report on American airlines")
    task_name = "Write a financial report for VEDL.NS"
    await Console(my_team_one.run_stream(task=task_name))
    await model_client.close()
    print("COMPLETED BEST SEARCH \n")


async def main():
    print("STARTED FROM THE MAIN FUNCTION\n")    
    await my_best_search()
